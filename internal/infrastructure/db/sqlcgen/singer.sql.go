...
const existsSinger = `-- name: ExistsSinger :one
SELECT EXISTS (
  SELECT 1 FROM singers WHERE id = $1
) AS exists
`

func (q *Queries) ExistsSinger(ctx context.Context, id int32) (bool, error) {
	row := q.db.QueryRowContext(ctx, existsSinger, id)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
const insertSinger = `-- name: InsertSinger :one
INSERT INTO singers (name)
VALUES ($1)
RETURNING id, name
`

func (q *Queries) InsertSinger(ctx context.Context, name string) (Singer, error) {
	row := q.db.QueryRowContext(ctx, insertSinger, name)
	var i Singer
	err := row.Scan(&i.ID, &i.Name)
	return i, err
}
